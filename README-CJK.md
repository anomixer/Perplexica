...
Vane 是一個**注重隱私的 AI 問答引擎**，完全在您自己的硬體上運行。它結合了來自網際網路的知識，支援**本地 LLM**（Ollama）和雲端供應商（OpenAI、Claude、Groq），在保持搜尋完全私密的同時，提供準確的答案並**引用來源**。

![preview](.assets/vane-screenshot.png)

想了解更多關於其架構和運作原理？請閱讀[這裡](https://github.com/anomixer/Vane/tree/master/docs/architecture/README.md)。

---

| 面向      | Perplexity（解惑機）                             | Vane (風向標)                                                  |
| ------- | ---------------------------------------------- | -------------------------------------------------------------------------- |
| 性質定位    | 商業化的 AI 搜尋與問答服務，直接提供雲端平台給使用者使用。​      | 開源 AI 搜尋引擎專案，**本fork已與上游 (v1.12.2) 同步，並包含修改與優化（修正 theme 與 CJK 支援）**。​                  |
...
#### 1. **Dark Theme 深色主題修復**
- ✅ **從上游合併最新架構** - 已同步至 v1.12.2，包含新版介面與功能
...
- ✅ **上游穩定度同步 (Upstream Sync)** - 整合了最新的 SearXNG 搜尋超時中斷 (Timeout) 與 Widget 錯誤捕捉機制，讓系統不再因為單一搜尋異常而卡死。
...
📄 **檔案上傳** - 上傳文件並詢問相關問題。PDF、文字檔案、圖片 - Vane 都能理解。
...
✨ **更多即將推出** - 我們正在根據社群反饋積極開發新功能。加入我們的 Discord 以幫助塑造 Vane 的未來！

---

## 📥 安裝方式

主要有兩種安裝 Vane 的方式 - 使用 Docker 和不使用 Docker。強烈建議使用 Docker。
...
docker run -d -p 3000:3000 \
  ...
  -v vane-uploads:/home/vane/uploads \
  --name vane \
...
- **日後更新**：在專案頁面點擊 `vane` -> 「動作」 -> 「重建 (Build)」(選取拉取映像檔) 即可。
...
這將拉取並啟動帶有捆綁 SearxNG 搜尋引擎的 Vane 容器。運行後，打開瀏覽器並導航至 http://localhost:3000。然後您可以直接在設置畫面中配置您的設定（API 金鑰、模型等）。

**注意**：映像包含 Vane 和 SearxNG，因此不需要額外設置。`-v` 標誌為您的數據和上傳的檔案創建持久卷。
...
Vane 建立在 AI 和大型語言模型應該讓每個人都容易使用的理念之上。如果您發現錯誤或有想法，請透過 GitHub Issues 分享。有關為 Vane 貢獻的更多資訊，您可以閱讀 [CONTRIBUTING.md](CONTRIBUTING.md) 文件以了解更多關於 Vane 以及如何為其做出貢獻。
...
感謝您探索 Vane，這個旨在增強您搜尋體驗的 AI 驅動搜尋引擎。我們正在不斷努力改進 Vane 並擴展其功能。我們重視您的反饋和貢獻，這有助於我們讓 Vane 變得更好。不要忘記回來查看更新和新功能！
